...
### 공식 문서 참조 원칙 (필수)
- **모든 wiki 페이지는 반드시 공식 문서를 1차 출처로 참조해야 한다**
- 블로그, 튜토리얼, 개인 정리글은 보조 자료로만 사용 — 단독 출처 금지
- 공식 문서 URL을 raw 원본 메타데이터와 wiki 페이지 상단에 명시
- 공식 문서가 없는 내용은 `> ⚠️ 비공식 출처:` 블록으로 표시
- 버전 정보를 반드시 함께 기록 (예: Spring Batch 6.0.3)

### 새 자료 입력 시
1. **공식 문서 URL 확인 후 우선 수집** — 공식 문서가 있으면 반드시 포함
2. `llm-wiki/raw/` 에 원본 저장 — **절대 수정 금지**
3. `llm-wiki/wiki/` 에 요약/합성 페이지 작성
4. 관련 기존 위키 페이지에 `[[wikilink]]` 추가
5. `llm-wiki/index.md` 갱신 (테이블 및 태그 인덱스)
6. `llm-wiki/log.md` 에 처리 내역 기록

### 질문 처리 시
- 가치 있는 질문 결과는 새 wiki 페이지로 저장
- 기존 페이지가 있으면 내용 갱신 (최초 작성일 유지, 갱신일 추가)

### 충돌 처리
- 상충하는 주장은 `> ⚠️ 충돌:` 블록으로 명시
- 출처를 함께 기록

### 파일명 규칙
- raw: `<주제>-<출처약어>-<연도>.md`
- wiki: 케밥케이스 영문 또는 한글 (`llm-wiki-methodology.md`)

### 연결 원칙
- 관련 개념은 `[[경로/파일명]]` 형식으로 연결
- 고아 페이지 최소화 — 새 페이지 생성 시 index.md에 반드시 등록
